    ...
    <div class="listing__content">
      ...
      <div class="listing__quantity-container">
        ...
        <div class="listing__quantity-buttons">
          <button class="listing__quantity-button" @click="decrementQuantity">
            -
          </button>
          <input
            class="listing__quantity-input"
            type="number"
            :value="quantity"
            @input="updateQuantity(+event.target.value)"
            min="0"
          />
          <button class="listing__quantity-button" @click="incrementQuantity">
            +
          </button>
        </div>
      </div>
    </div>
    <button class="listing__delete-button" @click="deleteListing">
      Delete
    </button>
  </div>
</template>
  ...
  methods: {
    async updateQuantity(quantity) {
      const listingRef = doc(
        ...
        String(this.$parent.useremail) + "listings",
        this.name
      );
